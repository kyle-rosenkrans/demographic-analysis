"""
Compute 5/10/15-min drive-time demographic rings for every school in
universal_schools.geojson. Outputs a large rollup keyed by school id that
the web UI uses when a user picks any school from the district.

Same methodology as etl/06_aggregate.py — great-circle approximation at
22 mph, combining both counties' ACS block-group data.

Inputs:
  data/processed/universal_schools.geojson
  data/processed/acs_broward.json, acs_miamidade.json
  data/processed/broward_blockgroups.geojson, miamidade_blockgroups.geojson

Output:
  data/processed/universal_rings.json  — {schoolId: {5min:{...},10min:{...},15min:{...}}}
"""
import json, math
...
print(f"Computing rings for {len(schools['features'])} schools...")
universal_rings = {}
MAX_RADIUS = max(m for _, m in RINGS) + 0.2
for i, sch in enumerate(schools["features"]):
    sid = sch["properties"]["id"]
    coord = sch["geometry"]["coordinates"]
    # Pre-filter BGs within max radius
    ...
    universal_rings[sid] = entry
    if (i+1) % 100 == 0:
        print(f"  {i+1}/{len(schools['features'])} ...")

with open("data/processed/universal_rings.json", "w") as f:
    json.dump(universal_rings, f)
print(f"-> data/processed/universal_rings.json  ({len(universal_rings)} schools)")
